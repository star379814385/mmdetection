...
# sit = torch.jit.script_if_tracing if torch_version_mid >= 8 else torch.jit._script_if_tracing
# @sit
# def get_new_zeros(tensor: torch.Tensor):
#     return torch.zeros_like(tensor)
#
# @sit
# def new_full_int(t: torch.Tensor, size: tuple, fill_value: int):
#     return t.new_full(size, fill_value)

# @sit
# def new_zeros(t: torch.Tensor, size: tuple):
#     return t.new_zeros(*size)
...
class ConvKernelHead(nn.Module):
    ...
    def _get_target_single(self, pos_inds, neg_inds, pos_mask, neg_mask,
        ...
        labels = pos_mask.new_full((num_samples, ),
                                   self.num_classes,
                                   dtype=torch.long)
        # labels = new_full_int(pos_mask, (num_samples, ), self.num_classes).to(torch.long)
        
        label_weights = pos_mask.new_zeros(num_samples)
        # label_weights = new_zeros(pos_mask, (num_samples, ))
        mask_targets = pos_mask.new_zeros(num_samples, H, W)
        # mask_targets = new_zeros(pos_mask, (num_samples, H, W))
        mask_weights = pos_mask.new_zeros(num_samples, H, W)
        # mask_weights = new_zeros(pos_mask, (num_samples, H, W))
        
        seg_targets = pos_mask.new_full((H, W),
                                        self.num_classes,
                                        dtype=torch.long)
        # seg_targets = new_full_int(pos_gt_mask, (H, W), self.num_classes).to(device=torch.long)
    ...
    def get_targets(self,
        ...
        if gt_sem_seg is None:
            gt_sem_seg = [None] * len(sampling_results)
            gt_sem_cls = [None] * len(sampling_results)
        results = multi_apply(
